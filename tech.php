...
/**
 * Récupère les données de l'API NASA APOD (Astronomy Picture of the Day).
 *
 * @param string $apiKey La clé API pour accéder à l'API NASA.
 * @param string $date La date pour laquelle récupérer les données. La date doit être au format 'YYYY-MM-DD'.
 *
 * @return array|null Les données de l'API NASA APOD sous forme de tableau associatif.
 *                      Si une erreur survient lors de la récupération des données, la fonction renvoie null.
 */
...
/**
 * Récupère les données de géolocalisation de l'API GeoPlugin pour une adresse IP donnée.
 *
 * @param string $ip L'adresse IP pour laquelle récupérer les données de géolocalisation.
 *
 * @return array Un tableau associatif contenant les informations de géolocalisation :
 *               - 'city' : la ville.
 *               - 'country' : le pays.
...
/**
 * Récupère les données de géolocalisation de l'API ipinfo.io pour une adresse IP donnée.
 *
 * @param string $ip L'adresse IP pour laquelle récupérer les données de géolocalisation.
 *
 * @return array Un tableau associatif contenant les informations de géolocalisation :
 ...
 *               - 'region' : la région.
 *               - 'country' : le pays.
 *               - 'location' : les coordonnées de la forme 'latitude,longitude'.
 */
...
// 4. WhatIsMyIP
/**
 * Récupère les données de géolocalisation de l'API WhatIsMyIP pour une adresse IP donnée.
 *
 * @param string $apikey4 La clé API pour accéder à l'API WhatIsMyIP.
 * @param string $ip4 L'adresse IP pour laquelle récupérer les données de géolocalisation.
 *
 * @return array Un tableau associatif contenant les informations de géolocalisation :
 *               - 'city' : la ville.
 *               - 'country' : le pays.
 *               Si une erreur survient lors de la récupération des données, la fonction renvoie un tableau avec
 *               'city' et 'country' à 'Non détectée'.
 */
